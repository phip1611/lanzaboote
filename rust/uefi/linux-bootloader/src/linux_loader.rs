...
impl InitrdLoader {
    ...
    pub fn new(boot_services: &BootServices, handle: Handle, initrd_data: Vec<u8>) -> Result<Self> {
        ...
        // path.
        unsafe {
            let initrd_proto_dp_ptr = init_or_get_linux_initrd_device_path()
                .as_ffi_ptr()
                .cast_mut()
                .cast::<c_void>();
...
///
/// Idempotent function.
fn init_or_get_linux_initrd_device_path() -> &'static DevicePath {
    if LINUX_INITRD_DEVICE_PATH.get().is_some() {
        log::debug!("LINUX_INITRD_DEVICE_PATH already initialized");
    }
    LINUX_INITRD_DEVICE_PATH.get_or_init(|| {
        let mut vec = Vec::new();
        {
